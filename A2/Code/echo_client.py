...
import logging
logging.basicConfig(filename='./err.log', filemode='w', level=logging.INFO, format='%(asctime)s %(relativeCreated)6d %(threadName)s %(message)s', datefmt='%m-%d %H:%M:%S')
...
def enlarge():
    outgoing = b'This is the message. hello, server! '
    for i in range(5):
...
def datatrans(keep_running, sock, myselect, alive, times):
   ...
   #         b'This is the message. hello, server'
   #         ]
...
#                        myselect.modify(sock, selectors.EVENT_READ)
#                        print("********************     %d" % alive)
#                        logging.info('get data from I/O : %s' % data)
#                        if alive == times:
#                            print("################     %d" % alive)
#                            keep_running = False
                    #return connection
